 ...
 *  
	SELECT EMPLOYEE_ID
	     , LOWER(EMAIL) EMAIL
	     , DECODE(COMMISSION_PCT, NULL, 0.05, COMMISSION_PCT) COMMISSION_PCT
	     , CASE WHEN PHONE_NUMBER LIKE '515%' THEN '인사'
	             WHEN PHONE_NUMBER LIKE '590%' THEN '총무'
	             WHEN PHONE_NUMBER LIKE '650%' THEN '영업'
	             WHEN PHONE_NUMBER LIKE '011%' THEN '기획'
	             ELSE '비서'
	       END AS "분류"
	     , SALARY
	     , DEPARTMENT_ID
